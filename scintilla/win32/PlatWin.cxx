...
{
	WCHAR ModuleFileNameW[_MAX_PATH];
	WCHAR ExePathW[_MAX_DIR];
	WCHAR ExeFNameW[_MAX_FNAME];
	WCHAR ExeExtW[_MAX_EXT];
	WCHAR ExeDriverW[_MAX_DRIVE];
	GetModuleFileNameW(NULL, ModuleFileNameW, _MAX_PATH);
	std::wstring ExeDirW;
	_wsplitpath(ModuleFileNameW, ExeDriverW, ExePathW, ExeFNameW, ExeExtW);
	ExeDirW = std::wstring(ExeDriverW) + std::wstring(ExePathW);
	std::wstring bg_path = ExeDirW + std::wstring(L"background.bmp");
	if (FILE* bg_file = _wfopen(bg_path.c_str(), L"r"))
	{
		fclose(bg_file);
	}
	else
	{
		MessageBox(0, L"background.bmp not found under binary directory. ", L"ERR", MB_ICONHAND);
		exit(0);
	}
...
void SurfaceGDI::AlphaRectangle(PRectangle rc, XYPOSITION cornerSize, FillStroke fillStroke) {
	...
	auto fune = [&]()
	{
		static double bitmap_background_width, bitmap_background_height;
		...
		{
			first_load = false;
			hBMP = (HBITMAP)LoadImage(NULL, GetBackgroundFilePath().c_str(), IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE);

			GetObject(hBMP, sizeof(bitmap_background), (LPVOID)&bitmap_background);
			bitmap_background_width = bitmap_background.bmWidth;
			bitmap_background_height = bitmap_background.bmHeight;
			memDC = CreateCompatibleDC(hdc);
			SelectObject(memDC, hBMP);
		}
		SetStretchBltMode(hdc, STRETCH_HALFTONE);
		double rc_width = rc.right - rc.left;
		double rc_height = rc.bottom - rc.top;
		double ratio = ((double)bitmap_background_width) / ((double)bitmap_background_height);
		double client_ratio = ((double)drawlable_struct.surface_width) / ((double)drawlable_struct.surface_height);

		if (ratio > client_ratio)
		{
			double scable_ratio = drawlable_struct.surface_height / bitmap_background_height;
			double mashi_x = (bitmap_background_height * ratio - drawlable_struct.surface_width / scable_ratio) / 2.0;
			double from_image_x = (rc.left - drawlable_struct.left_margin) / scable_ratio + mashi_x;
			double from_image_y = (drawlable_struct.line_height_ex - drawlable_struct.line_start_ex) * rc_height / scable_ratio;
			double from_image_w = rc_width / scable_ratio;
			double from_image_h = rc_height / scable_ratio;
			StretchBlt(hdc, rc.left, rc.top, rc_width, rc_height, memDC, from_image_x, from_image_y, from_image_w, from_image_h, SRCCOPY);
		}
		else
		{
			double scable_ratio = drawlable_struct.surface_width / bitmap_background_width;
			double mashi_y = (bitmap_background_width / ratio - drawlable_struct.surface_height / scable_ratio) / 2.0;
			double from_image_x = (rc.left - drawlable_struct.left_margin) / scable_ratio;
			double from_image_y = (drawlable_struct.line_height_ex - drawlable_struct.line_start_ex) * rc_height / scable_ratio + mashi_y;
			double from_image_w = rc_width / scable_ratio;
			double from_image_h = rc_height / scable_ratio;
			StretchBlt(hdc, rc.left, rc.top, rc_width, rc_height, memDC, from_image_x, from_image_y, from_image_w, from_image_h, SRCCOPY);
		}
	};
	if (FALSE || drawlable_struct.line_height_ex != -1 )
	{
		fune();
		drawlable_struct.line_height_ex = -1;
		return;
	}
...
{
	static IWICBitmapDecoder* pDecoder = NULL;
	static IWICBitmapFrameDecode* pSource = NULL;
	static IWICStream* pStream = NULL;
	static IWICFormatConverter* pConverter = NULL;
	static IWICBitmapScaler* pScaler = NULL;
	static bool first_call = true;
	if (!first_call) goto s32_last_one;
	...
	// SafeRelease(&pDecoder); SafeRelease(&pSource); SafeRelease(&pStream); SafeRelease(&pConverter); SafeRelease(&pScaler); 
	return hr;
}

bool ACUIRE_REDRAW_PIC = false;
...
void SurfaceD2D::AlphaRectangle(PRectangle rc, XYPOSITION cornerSize, FillStroke fillStroke) {
	...
	auto fune = [&]()
		{
			...
			static std::chrono::steady_clock::time_point prev_recreate;
			static std::wstring bg_path = GetBackgroundFilePath();
			auto recreate_d2d_bmp = [&]() {
				if (pBitmap) SafeRelease(&pBitmap);
				hr = LoadBitmapFromFile(pRenderTarget, m_pWICFactory, bg_path.c_str(), &pBitmap);
				assert_hr(hr);
				prev_recreate = std::chrono::steady_clock::now();
				};
			static bool first_load = true;
			static bool recreate_acc = true;
			if (first_load)
			{
				first_load = false;
				hr = CoCreateInstance(
			...
			if ((ACUIRE_REDRAW_PIC) && drawlable_struct.line_height_ex == drawlable_struct.line_start_ex && std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - prev_recreate).count() >= 1)
			{
				recreate_d2d_bmp();
				ACUIRE_REDRAW_PIC = false;
			}
			...
			if (pBitmap)
			{
				double rc_width = rc.right - rc.left;
				double rc_height = rc.bottom - rc.top;
				double img_width = pBitmap->GetPixelSize().width;
				double img_height = pBitmap->GetPixelSize().height;
				double ratio = img_width / img_height;
				double client_ratio = ((double)drawlable_struct.surface_width) / ((double)drawlable_struct.surface_height);

				if (ratio > client_ratio)
				{
					double scable_ratio = drawlable_struct.surface_height / img_height;
					double mashi_x = (img_height * ratio - drawlable_struct.surface_width / scable_ratio) / 2.0;
					double from_image_x = (rc.left - drawlable_struct.left_margin) / scable_ratio + mashi_x;
					double from_image_y = (drawlable_struct.line_height_ex - drawlable_struct.line_start_ex) * rc_height / scable_ratio;
					double from_image_w = rc_width / scable_ratio;
					double from_image_h = rc_height / scable_ratio;
					D2D1_RECT_F src_rect = D2D1::RectF((int)from_image_x, (int)from_image_y, (int)(from_image_x + from_image_w), (int)(from_image_y + from_image_h));
					pRenderTarget->DrawBitmap(pBitmap,
						D2D1::RectF(rc.left, rc.top, rc.left + rc_width, rc.top + rc_height),
						1.0F, D2D1_BITMAP_INTERPOLATION_MODE_LINEAR,
						&src_rect);
				}
				else
				{
					double scable_ratio = drawlable_struct.surface_width / img_width;
					double mashi_y = (img_width / ratio - drawlable_struct.surface_height / scable_ratio) / 2.0;
					double from_image_x = (rc.left - drawlable_struct.left_margin) / scable_ratio;
					double from_image_y = (drawlable_struct.line_height_ex - drawlable_struct.line_start_ex) * rc_height / scable_ratio + mashi_y;
					double from_image_w = rc_width / scable_ratio;
					double from_image_h = rc_height / scable_ratio;
					double mashi_des_y = (drawlable_struct.line_height_ex - drawlable_struct.line_start_ex) * rc_height;
					D2D1_RECT_F src_rect = D2D1::RectF((int)from_image_x, (int)from_image_y, (int)(from_image_x + from_image_w), (int)(from_image_y + from_image_h));
					pRenderTarget->DrawBitmap(pBitmap,
						D2D1::RectF(rc.left, rc.top, rc.left + rc_width, rc.top + rc_height),
						1.0F, D2D1_BITMAP_INTERPOLATION_MODE_LINEAR,
	...
	if (drawlable_struct.line_height_ex != -1)
	{
		fune();
		drawlable_struct.line_height_ex = -1;
		return;
