...
ColourRGBA transRGBA(255, 255, 255, 0);
...
void FillLineRemainder(Surface *surface, const EditModel &model, const ViewStyle &vsDraw, const LineLayout *ll,
	...
	if (eolInSelection && vsDraw.selection.eolFilled && (line < model.pdoc->LinesTotal() - 1) && (vsDraw.selection.layer == Layer::Base)) {
		...
		surface->drawlable_struct.line_height_ex = line;
		surface->FillRectangleAligned(rcArea, Fill(transRGBA));
		if (eolInSelection && vsDraw.selection.eolFilled && (line < model.pdoc->LinesTotal() - 1) && (vsDraw.selection.layer != Layer::Base)) {
...
void EditView::DrawEOL(Surface *surface, const EditModel &model, const ViewStyle &vsDraw, const LineLayout *ll,
	...
	if (eolInSelection && (line < model.pdoc->LinesTotal() - 1) && (vsDraw.selection.layer == Layer::Base)) {
		...
		if (background) {
			...
			surface->drawlable_struct.line_height_ex = line;
			surface->FillRectangleAligned(rcSegment, Fill(transRGBA));
...
void DrawBackground(Surface *surface, const EditModel &model, const ViewStyle &vsDraw, const LineLayout *ll,
	...
	while (bfBack.More()) {
		...
		if (!horizontal.Empty() && rcLine.Intersects(horizontal)) {
			...
			//ColourRGBA textBack = TextBackground(model, vsDraw, ll, ColourRGBA(0,255,0,255), inSelection, inHotspot, ll->styles[i], i);
			ColourRGBA textBack = TextBackground(model, vsDraw, ll, transRGBA, inSelection, inHotspot, ll->styles[i], i);
			if (ts.representation) {
				if (ll->chars[i] == '\t') {
					// Tab display
				...
				surface->drawlable_struct.line_height_ex = p_line_ex_3;
				surface->FillRectangleAligned(rcSegment, Fill(textBack));
			} else {
				// Normal text display
				surface->drawlable_struct.line_height_ex = p_line_ex_3;
...
void EditView::DrawLine(Surface *surface, const EditModel &model, const ViewStyle &vsDraw, const LineLayout *ll,
	...
	surface->drawlable_struct.left_margin = rcLine.left;
		...
		for (const DrawPhase &phase : phases) {
			...
			while (visibleLine < model.pcs->LinesDisplayed() && yposScreen < rcArea.bottom) {
				...
				if (ll) {
					...
					surface->drawlable_struct.surface_height = surfaceWindow->drawlable_struct.surface_height;
					surface->drawlable_struct.surface_width = surfaceWindow->drawlable_struct.surface_width;
					surface->drawlable_struct.line_start_ex = surfaceWindow->drawlable_struct.line_start_ex;
